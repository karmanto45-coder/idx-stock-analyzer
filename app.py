...
# ================================================================
# PILIHAN PERIODE ANALISIS MUSIMAN (SEASONALITY) ← FITUR BARU
# ================================================================
SEASONALITY_PERIOD_OPTIONS = {
    "5 Tahun (default)": "5y",
    "10 Tahun": "10y",
    "15 Tahun": "15y",
    "20 Tahun": "20y",
}
SEASONALITY_PERIOD_NOTES = {
    "5y": (
        "info",
        "Paling relevan dengan kondisi pasar & regulasi BEI saat ini, tapi jumlah "
        "observasi per bulan lebih sedikit (~5 titik data/bulan) sehingga status "
        "'reliable' lebih sulit tercapai untuk pola bulanan.",
    ),
    "10y": (
        "info",
        "Keseimbangan yang cukup baik antara relevansi pasar terkini dan jumlah "
        "sampel (~10 titik data/bulan). Cocok dipakai sebagai default alternatif.",
    ),
    "15y": (
        "warning",
        "Sampel makin banyak & pola musiman makin stabil secara statistik, tapi mulai "
        "mencampur beberapa rezim pasar berbeda (misal sebelum & sesudah perubahan "
        "lot size 2014). Data historis saham yang IPO belakangan tidak akan sepanjang ini.",
    ),
    "20y": (
        "warning",
        "Sampel paling banyak (paling 'meyakinkan' secara statistik murni), tapi mencampur "
        "krisis 2008, era pra-2014, hingga sekarang — pola yang muncul bisa jadi tidak lagi "
        "mencerminkan perilaku pasar & investor saat ini. Banyak saham non-blue-chip tidak "
        "akan punya data sepanjang ini (otomatis dipotong ke histori yang tersedia).",
    ),
}

with st.sidebar:
    st.markdown("## 📆 Periode Analisis Musiman")
    _season_label = st.selectbox(
        "Periode data untuk Pola Waktu Historis/Terlemah",
        list(SEASONALITY_PERIOD_OPTIONS.keys()),
        index=0,
        help=(
            "Hanya memengaruhi bagian 'Pola Waktu Historis' & 'Pola Waktu Terlemah'. "
            "Indikator teknikal, skor, dan forecast harga TETAP memakai data 5 tahun "
            "seperti biasa, tidak terpengaruh pilihan ini."
        ),
    )
    SEASONALITY_PERIOD = SEASONALITY_PERIOD_OPTIONS[_season_label]
    _note_kind, _note_text = SEASONALITY_PERIOD_NOTES[SEASONALITY_PERIOD]
    (st.warning if _note_kind == "warning" else st.info)(_note_text)

    if SEASONALITY_PERIOD != "5y":
        st.caption(
            "⚠️ **Peringatan throttling Yahoo Finance/Streamlit**: memilih periode lebih "
            "dari 5 tahun berarti request tambahan ke Yahoo Finance untuk SETIAP saham yang "
            "dianalisis. Di tab **Screening Multi-Saham** dan **Rekomendasi Bullish Harian** "
            "(yang memproses banyak saham sekaligus dalam satu klik), ini bisa memicu Yahoo "
            "Finance membatasi/menolak request (HTTP 429) karena Streamlit Community Cloud "
            "memakai IP keluar yang dipakai bersama banyak aplikasi lain — jadi lebih rawan "
            "kena limit dibanding menjalankan yfinance dari komputer pribadi. Data yang sudah "
            "pernah diambil akan di-cache 1 jam, tapi permintaan pertama untuk ticker baru "
            "tetap kena risiko ini. **Saran**: gunakan periode >5 tahun hanya di tab "
            "'Analisis Satu Saham' atau 'Decision Engine' (satu ticker per klik), dan tetap "
            "pakai 5/10 tahun untuk tab yang memproses banyak saham sekaligus."
        )
    st.divider()
...
def analyze_ticker(ticker: str, horizon_label: str, seasonality_period: str = "5y"):
    ...
    # Simpulan kualitatif kesehatan & valuasi (untuk tab Decision Engine) ← FITUR BARU
    ...
    # Pola musiman (seasonality) dihitung dari series HARGA TERPISAH sesuai periode
    # yang dipilih pengguna di sidebar — TIDAK memengaruhi df utama (indikator/skor/
    # forecast tetap konsisten pakai 5y seperti semula). Kalau periode yang dipilih
    # sama dengan 5y, pakai ulang df["Close"] yang sudah ada supaya tidak fetch dobel. ← FITUR BARU
    if seasonality_period == "5y":
        seasonality_close = df["Close"]
    else:
        df_season = fetch_ohlcv(ticker, period=seasonality_period)
        seasonality_close = df_season["Close"] if not df_season.empty else df["Close"]
    timing = best_timing_summary(seasonality_close)
...
def get_gemini_api_key() -> str | None:
    """
    Ambil Gemini API key dengan urutan prioritas:
    1. st.secrets["GEMINI_API_KEY"]     <- Streamlit Cloud Secrets (direkomendasikan)
    2. os.environ["GEMINI_API_KEY"]     <- environment variable lokal
    3. st.session_state["_gemini_key_override"] <- input manual dari UI (fallback)
    """
    import os
    try:
    ...
    if key:
        return key
    return st.session_state.get("_gemini_key_override", "").strip() or None


def call_claude_api(prompt: str, system: str = "") -> str:
    """
    Panggil Google Gemini API (gratis, tersedia di Indonesia).
    API key dibaca otomatis dari Streamlit Secrets -> env var -> input manual.
    Model: gemini-2.0-flash (gratis, cepat, cukup untuk narasi analisis).
    """
    api_key = get_gemini_api_key()
    if not api_key:
        return (
            "⚙️ API key Gemini belum dikonfigurasi. "
            "Tambahkan GEMINI_API_KEY ke Streamlit Secrets, "
            "atau isi di kotak API Key pada sidebar."
        )
    ...
    try:
        ...
        if resp.status_code == 200:
            ...
            try:
                ...
                return "⚠️ Respons AI tidak terbaca, coba lagi."
        elif resp.status_code == 400:
            return "❌ API key Gemini tidak valid. Periksa kembali key di Streamlit Secrets."
        elif resp.status_code == 429:
            return "⏳ Rate limit Gemini tercapai. Tunggu 1 menit lalu coba lagi."
        else:
            return f"❌ Gemini API error HTTP {resp.status_code}: {resp.text[:200]}"
...
# ── Sidebar: Konfigurasi AI (Gemini) ─────────────────────────────────────
with st.sidebar:
    st.markdown("## ⚙️ Konfigurasi AI")
    _key_status = get_gemini_api_key()
    if _key_status:
        st.success("✅ Gemini API key aktif — fitur AI siap", icon="🤖")
    else:
        st.info(
            "**Narasi AI menggunakan Google Gemini** (gratis).\n\n"
            "**Cara mendapatkan key (gratis):**\n"
        ...
        st.divider()
        st.markdown("**Cara pasang di Streamlit Cloud:**")
        st.code('GEMINI_API_KEY = "AIza..."', language="toml")
        st.caption("Settings → Secrets → paste baris di atas → Save changes")
        st.divider()
        st.markdown("**Atau isi langsung di sini** *(sesi ini saja)*:")
        ...
        if _manual_key and _manual_key.startswith("AIza"):
            st.session_state["_gemini_key_override"] = _manual_key
            st.success("Key aktif untuk sesi ini ✅")
        elif _manual_key:
            st.error("Format tidak valid. Key Gemini diawali AIza...")
    st.divider()
    st.markdown(
...
with tab1:
    ...
    if run:
        ...
        for i, tk in enumerate(tickers):
            ...
            try:
                r = analyze_ticker(tk, horizon_label, SEASONALITY_PERIOD)
                if r:
                    results.append(r)
            except Exception as e:
        ...
        if not results:
            ...
            st.caption(
                f"Pola musiman dihitung dari data {_season_label.lower()} terakhir. "
...
with tab2:
    ...
    if st.button("Analisis", type="primary"):
        with st.spinner(f"Mengambil & menganalisis {ticker}..."):
            r = analyze_ticker(ticker, horizon_label2, SEASONALITY_PERIOD)
        if r is None:
            ...
            st.caption(f"Berdasarkan data {_season_label.lower()} terakhir.")
            tw = r["timing"]
...
with tab4:
    ...
    if run_daily:
        ...
        for i, tk in enumerate(daily_tickers):
            ...
            try:
                r = analyze_ticker(tk, daily_horizon, SEASONALITY_PERIOD)
                if r is None:
                    continue
        ...
        if not bullish_results:
            ...
            for idx, (r, signals, sig_count) in enumerate(bullish_results):
                ...
                with st.expander(f"{color} **{ticker_name}** — Skor {score} | Rp {price:,.0f} | {sig_count} sinyal bullish"):
                    ...
                    st.caption(
                        f"📅 (data {_season_label.lower()}) Historis paling sering naik: hari **{wd.get('name','?')}** "
...
with tab5:
    ...
    if st.button("⚡ Analisis & Buat Keputusan", type="primary", use_container_width=True):
        with st.spinner(f"Menganalisis {de_ticker}..."):
            r_de = analyze_ticker(de_ticker, de_horizon, SEASONALITY_PERIOD)
        ...
        if r_de is None:
            ...
            # Kesehatan Fundamental & Valuasi — Sehat/Undervalue ← FITUR BARU
            fv = r_de.get("fund_verdict", {})
            st.markdown("#### 💊 Kesehatan Fundamental & Valuasi")
            _verdict_box = {
                "good": st.success, "warning": st.warning,
                "bad": st.error, "unknown": st.info, "neutral": st.info,
            }.get(fv.get("verdict_key", "unknown"), st.info)
            _verdict_box(fv.get("verdict", "❔ Data Fundamental Tidak Tersedia"))

            fh1, fh2 = st.columns(2)
            fh1.metric(
                "Kesehatan Perusahaan", fv.get("health_label", "?"),
                help=f"Berdasarkan {fv.get('n_health_checks', 0)} indikator: ROE, DER, pertumbuhan laba",
            )
            fh2.metric(
                "Valuasi Harga", fv.get("valuation_label", "?"),
                help=f"Berdasarkan {fv.get('n_value_checks', 0)} indikator: PER, PBV",
            )

            with st.expander("Lihat rincian penilaian fundamental"):
                for reason in fv.get("reasons", []):
                    st.markdown(f"• {reason}")
                fund_de = r_de.get("fundamentals", {})
                if fund_de:
                    fc1, fc2, fc3, fc4, fc5 = st.columns(5)
                    fc1.metric("PER", f"{fund_de.get('pe'):.1f}x" if fund_de.get("pe") else "n/a")
                    fc2.metric("PBV", f"{fund_de.get('pbv'):.2f}x" if fund_de.get("pbv") else "n/a")
                    fc3.metric("ROE", f"{fund_de.get('roe')*100:.1f}%" if fund_de.get("roe") is not None else "n/a")
                    fc4.metric("DER", f"{fund_de.get('debt_to_equity'):.0f}%" if fund_de.get("debt_to_equity") is not None else "n/a")
                    fc5.metric("Pertumbuhan Laba (YoY)", f"{fund_de.get('eps_growth')*100:.1f}%" if fund_de.get("eps_growth") is not None else "n/a")
            st.caption(
                "⚠️ Ini heuristik sederhana berbasis ambang batas umum (PER<15x, PBV<1.5x, ROE>15%, "
                "DER<100%, laba tumbuh), BUKAN riset ekuitas menyeluruh, BUKAN valuasi DCF, dan tidak "
                "dibandingkan dengan rata-rata sektor. Data Yahoo Finance untuk emiten IDX non blue-chip "
                "sering tidak lengkap. Selalu cross-check dengan laporan keuangan resmi & riset broker "
                "sebelum memutuskan. Bukan saran investasi."
            )
            ...
            st.caption(f"Berdasarkan data {_season_label.lower()} terakhir (bisa diubah di sidebar).")
