...
def best_timing_summary(close: pd.Series) -> dict:
    ...
    def bottom(df, min_obs):
        """
        Kebalikan dari top(): karena tabel sudah diurutkan descending
        berdasarkan avg_return, baris TERAKHIR = avg_return TERENDAH
        (secara historis paling sering/paling dalam menurun). Memakai
        kolom & tabel yang sama persis dengan top() agar "terkuat" dan
        "terlemah" konsisten dihitung dari data historis yang identik.
        """
        row = df.iloc[-1]
        return {
            "name": df.index[-1],
    ...
    return {
        ...
        "worst_weekday": bottom(wd, MIN_OBS_WEEKDAY),
        "worst_bucket": bottom(bk, MIN_OBS_BUCKET),
        "worst_month": bottom(mo, MIN_OBS_MONTH),
        "weekday_table": wd,
        "bucket_table": bk,
        "monthly_table": mo,
    }
